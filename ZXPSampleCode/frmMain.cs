    ...
    public partial class frmMain : Form
    {
        ...
        private void AddSmartCardType(ref ComboBox cbo, string cardType, ref bool first)
        {
            try
            {
                ...
                {
                    if (cardType.ToLower().Contains("mifare") || cardType.ToLower().Contains("felica") ||
                        cardType.ToLower().Contains("contactless"))
                        AddCardType(ref cbo, cardType, ref first);
                }
                else if (item.Contains("Contact"))
                {
                    if (cardType.ToLower().Contains("pvc,sle") || cardType.ToLower().Contains("contact"))
                        AddCardType(ref cbo, cardType, ref first);
                }
                else 
                {
                    if (cardType.ToLower().Contains("pvc,sle") || cardType.ToLower().Contains("mifare") || 
                        cardType.ToLower().Contains("contactless") || cardType.ToLower().Contains("contact"))
                    {
        ...
        private void AddMagneticCardType(ref ComboBox cbo, string cardType, ref bool first)
        {
            try
            {
                if (cardType.ToLower().Contains("loco") || cardType.ToLower().Contains("hico") || cardType.ToLower().Contains("mag"))
                {
                    cbo.Items.Add(cardType);
                    if (first)
                    {
        ...
        private void AddPrintCardType(ref ComboBox cbo, string cardType, ref bool first)
        {
            try
            {
                switch (cardType.ToLower())
                {
                    ...
                    case "composite,z6":
                    case "comp,re-xfer rdy":
                    case "pvc,standard":
                    case "composite,standard":
                        cbo.Items.Add(cardType);
                        if (first)
                        {
                            cbo.Text = cardType;
